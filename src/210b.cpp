...
using namespace std;
class Solution {
  bool dsf(int course, vector<vector<int>> &graph, vector<int> &state, vector<int> &order) {
    state[course] = 1; // Being explored
    //
...
public:
  vector<int> findOrder(int numCourses, vector<vector<int>> &prerequisites) {
    // This is typical topological sort over a disjoint graph.
    // If there is a cycle it's still not possible to pass all courses.
    ...
    for (int course = 0; course < numCourses; course++) {
      if (state[course] == 0) {
        if (dsf(course, graph, state, order)) {
